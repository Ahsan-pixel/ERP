...
$(document).on('click', 'table.ajax_view tbody tr', function(e) {
    if (
        ...
        !$(e.target).is('a') &&
        !$(e.target).is('input.custom-checkbox') &&
        !$(e.target).is('button') &&
...
function checkboxClicked(checkbox, id, isChecked,ID) {
    ...
    $.ajax({
        url: '/markitactive',
        method: 'POST',
        data: { id: id, newState: !currentStatus },
        success: function(response) {
            if(!currentStatus){
            document.getElementById('span_'+id).innerText = 'Acknowledged';
            document.getElementById('span_'+id).style.backgroundColor = 'green';
            }

            else
            {
            document.getElementById('span_'+id).innerText = 'Pending';
...
function checkboxClickedUser(checkbox, id) {
console.log(checkbox, id )
 
    $.ajax({
        url: '/markitactiveTicket',
        method: 'POST',
        data: { id: id, newState: checkbox },
        success: function(response) {
...
function callmyfunctionsendmail(emailContent ) {
    // Convert isChecked to boolean

    var id = document.getElementById("idValue").innerText;
    var email = document.getElementById("toAddressValue").innerText;
    // var emailContent = document.getElementById("emailContent").value;
    
    document.getElementById("Sending-mail-btn").innerText= 'sending';
  
    $.ajax({
        url: '/sendMailCompletion',
        method: 'POST',
        data: { id: id, email: email ,emailContent:emailContent },
        success: function(response) {
            // document.getElementById("loader").style.display = "none";
            document.getElementById("Sending-mail-btn").innerText= 'sending';
            document.getElementById("myModal").style.display="none";
            alert('Mail sent successfully')
            document.getElementById('span_mail_'+id).innerText = 'send';
            document.getElementById('span_mail_'+id).style.backgroundColor = 'green';
            // Handle the server response here
        },
        error: function(xhr, status, error) {
            // Handle errors here
...
function UploadImageHandler(image, id) {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: '/UploadImageinDatabase',
            method: 'POST',
            data: { id: id, image: image },
            success: function(response) {
                // If the response indicates success, resolve the promise
                ...
                if (response.success) {
                    ...
                    reject('Image upload failed'); // Reject the promise with an error message
                }
            },
            error: function(xhr, status, error) {
...
function resizeImage(file, maxWidth, maxHeight, callback) {
    ...
    reader.onload = function (event) {
        ...
        image.onload = function () {
            ...
            // Convert canvas to resized image data URL
