$(document).ready(function() {
    ...
    $('table#pos_table tbody').on('click', 'i.pos_return_row', function() {
        $(this)
            .parents('tr')
            .remove();
            pos_total_row();
            var row = $(this).closest('tr');
            var productId = row.find('input.product_ids').val();
            var sellLineNote = row.find('input.sells_lines_notes').val();
            var productName = row.find('input.products_names').val();


            
            // Send an AJAX request to update the transaction with the additional data
            $.ajax({
                url: '/update-new-transaction', // Backend route that handles the update
                ...
                error: function(xhr, status, error) {
                    alert('Something went wrong: ' + error); // Handle error
                }
            });
    });
